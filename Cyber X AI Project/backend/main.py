...
import logging
from models import POSLog, iso_forest
import csv
...
async def verify_user_token(token: str, request: Request):
    headers = {key: value for key, value in request.headers.items()}  # Retrieve headers as a dictionary
    print(headers)  # Print all headers to the console

    # Example token verification function
    verify_token(token=token)
    return {
        "message": "Token is valid",
        "headers": headers  # Optionally return the headers in the response
    }
...
async def upload_pos_logs(pos_log: RequestLog, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    # df['timestamp_numeric'] = 2  # Convert to Unix timestamp

    # is_anomalous = iso_forest.decision_function(df[['transaction_id', 'duration_time', 'item_code', 'amount', 'user_id', 'payment_method']])
    print(fields[1])
    if fields[1] == 'A123': 
        fields[1] = 0.
    elif fields[1] == 'B456': 
    ...
    is_anomalous = iso_forest.decision_function(fields)
    if is_anomalous == -1:
        is_anomalous = False
    else: 
        is_anomalous = True
    ...
    return {"message": "POS log added successfully", "pos_log_id": pos_log.id, "is_anomalous": is_anomalous}
